...
export function prepareBatch(items) {
  ...
  const promisifiedArr = []

  for (const item of items) {
    const { name, prepTime } = item

    const itemPromise = prepareOrder(name, prepTime)

    promisifiedArr.push(itemPromise)
  }
...
export function prepareSafeBatch(items) {
  ...
  const promisifiedArr = items.map(({ name, prepTime }) => {
    return prepareOrder(name, prepTime)
  })

  return Promise.allSettled(promisifiedArr).then((resultArr) => {
