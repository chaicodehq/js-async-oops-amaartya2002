...
export async function retryPayment(paymentFn, maxRetries, delayMs) {
  ...
  for (let tries = 0; tries <= maxRetries; tries++) {
    try {
      const data = await paymentFn()
      return data
    } catch (error) {
      if (error.message === new NetworkError().message) {
        setTimeout(() => { }, delayMs)
      } else {
...
export function categorizeError(error) {
  ...
  let errObj
  if (error instanceof InsufficientFundsError) {
    errObj = { type: "insufficient_funds", retryable: false, message: error.message }
  } else if (error instanceof NetworkError) {
    errObj = { type: "network", retryable: true, message: error.message }
  } else if (error instanceof FraudDetectedError) {
    errObj = { type: "fraud", retryable: false, message: error.message }
  } else {
    errObj = { type: "unknown", retryable: false, message: error.message || "Unknown error" }
  }
  return errObj
}
